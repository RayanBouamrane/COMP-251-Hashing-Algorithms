...
public class Chaining {
    ...
    public int chain(int key) {
        return ((((this.A * key) % (power2(w))) >> (w - r)));
    }
    ...
    public int insertKey(int key) {
        int index = chain(key);
        if (Table.get(index).isEmpty()) {       //if the slot is empty, we can be certain the
            (Table.get(index)).add(key);        //key is not present, and add safely
            return 0;
        } else {
            Table.get(index).add(key);          // element not present, add it
            return 1;
        }
